...
class CloudinaryHelper1 {
    ...
    static func upload(cloudinary: CLDCloudinary, url: URL, resourceType: CLDUrlResourceType) -> CLDUploadRequest {
        ...
        if (resourceType == CLDUrlResourceType.image) {
            ...
            return cloudinary.createUploader().uploadLarge(url: url, uploadPreset: "whqiudnc", params: params, preprocessChain: chain, chunkSize: 5 * 1024 * 1024)
        } else {
            return cloudinary.createUploader().uploadLarge(url: url, uploadPreset: "whqiudnc", params: params, chunkSize: 5 * 1024 * 1024)
        }
    }

    static func getThumbUrl(cloudinary: CLDCloudinary, resource: CloudResource) -> String {
    ...
    static func generateEffectList(cloudinary: CLDCloudinary, resource: CloudResource) -> [EffectMetadata] {
    ...
    static func generateVideoEffectList(cloudinary: CLDCloudinary, resource: CloudResource) -> [EffectMetadata] {
        var transformation: CLDTransformation!
        var result = [EffectMetadata]()
